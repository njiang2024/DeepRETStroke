...
def get_args_parser():
	...
	# Dataset parameters
	parser.add_argument('--data_path', default='draft_data/test/gray_fundus', type=str,
						help='dataset path')
...
def main_0(args):
	...
	if args.finetune and not args.eval:
		...
		checkpoint_model = checkpoint['model'] # 加载的模型权重字典
		# state_dict = model.state_dict() # 任务需要的模型权重模板
		...
		if args.combined:
			trunc_normal_(model.meta_layer.weight, std=2e-5)
			trunc_normal_(model.head_c2.weight, std=2e-5)
			trunc_normal_(model.head_c5.weight, std=2e-5)
			for name, para in model.named_parameters():
				if "meta_layer" in name or "head_c" in name:
					para.requires_grad = True
				else:
					para.requires_grad = False
		else:
			trunc_normal_(model.head_2.weight, std=2e-5)
	...
	if mixup_fn is not None:
		...
		if args.pre_train or args.progression == False:
			if args.smoothing > 0.:
	...
	if args.eval:
		test_stats,auc_roc,df_result = evaluate(data_loader_test, model, device, args.task, epoch=0, mode='test',
												pre_train=args.pre_train, progression=args.progression,
												combined=args.combined)
		result_path = os.path.join(args.task, "results.csv")
	...
	for epoch in range(args.start_epoch, args.epochs):
		...
		if max_auc<val_auc_roc:
			...
			if args.output_dir:
				misc.save_model(
					args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
					loss_scaler=loss_scaler, epoch=epoch, best_weight=True)
...
#############################################################################################
	...
	#dft_sb_te = pd.DataFrame([])
	#dft_st_te = pd.DataFrame([])
	...
	for repeat in range(3):
		...
		if repeat == 0:
			...
			checkpoint = torch.load(args.task + 'checkpoint-4-r%d-best.pth'%(repeat), map_location='cpu')
		...
		for epoch in range(1):
			...
			if max_auc_s4 < val_auc_roc:
				...
				if args.output_dir:
					misc.save_model(
						args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
						loss_scaler=loss_scaler, epoch=epoch, best_weight=True, stage=stage, repeat=repeat)
			else:
				ctimes = ctimes + 1
				if ctimes > ptimes:
